...
public class KTobaccoWaitToCommetOrderListAdapter extends BaseAdapter {
    ...
    public static interface OnOrderCommetClickListener {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.k_listview_waitto_commet_order_list_item, parent, false);
        }
        final Button btnCommet = (Button) convertView.findViewById(R.id.btn_commet);
        ...
        final TextView txtCallSender = (TextView) convertView.findViewById(R.id.deliverPhoneNumber);
        ...
        // 评价按钮处理
        if (!item.isCommeted() && onOrderCommetClickListener != null) {
            btnCommet.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    onOrderCommetClickListener.onOrderCommet(position, item);
                }
            });
        } else if (item.isCommeted()) {
            btnCommet.setEnabled(false);
            btnCommet.setText("已评价");
            btnCommet.setBackgroundResource(R.drawable.shape_gray_corner);
        }
        return convertView;
    }
