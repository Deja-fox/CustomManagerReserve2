...
import com.jald.reserve.bean.http.request.KLoginPwdUpdateRequestBean;
...
@ContentView(R.layout.k_activity_login_pwd_update)
public class KLoginPwdUpdateActivity extends KBaseActivity {
        ...
        if (checkInputField()) {
            ...
            DialogProvider.showProgressBar(this, new OnCancelListener() {
                ...
                public void onCancel(DialogInterface dialog) {
                    KHttpClient.singleInstance().cancel(KLoginPwdUpdateActivity.this);
                }
            });
            KLoginPwdUpdateRequestBean loginPwdUpdateBean = new KLoginPwdUpdateRequestBean();
            loginPwdUpdateBean.setOld_password(MD5Tools.MD5(this.oldPassword));
            loginPwdUpdateBean.setNew_password(MD5Tools.MD5(this.newPassword));

            KHttpClient.singleInstance().postData(KLoginPwdUpdateActivity.this, KHttpAdress.LOGIN_PWD_UPDATE, loginPwdUpdateBean, userInfoStub, new KHttpCallBack() {
                @Override
    ...
    private void onLoginPwdUpdateTransactionSuccess() {
        Toast.makeText(this, "恭喜您登录密码修改成功", Toast.LENGTH_SHORT).show();
        finish();
    }
